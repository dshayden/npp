...
def main(args):
  ...
  if o.lie == 'se2':
    ...
    if args.save:
      ...
      if args.single_frame:
        ...
        if args.world_axis:
          ...
          W = np.eye(4)
          W[1,3] = 0.75 # move y up
          W[2,3] = 1.25 # move z back

          scenes[t].add_geometry(tmu.MakeAxes(0.1, W,
            np.tile(gray, [4,1]).astype(np.int), minor=0.01))
          # scenes[t].add_geometry(tmu.MakeAxes(0.2, W,
          #   np.tile(gray, [4,1]).astype(np.int), minor=0.01))


        tmu.show_scene_with_bindings(scenes[t], res=[1920,1080], point_eps=args.point_eps)
    ...
    zColsAlpha = (255*du.diffcolors(100, bgCols=[[1,1,1],[0,0,0]], alpha=0.75)).astype(np.uint8)
    zColsFloatAlpha = du.diffcolors(100, bgCols=[[1,1,1],[0,0,0]], alpha=0.75)
    ...
    for t in range(T):
      mesh[t].visual.vertex_colors = zColsAlpha[z[t]]
    ...
    if not args.single_frame:
      ...
      t_rng = range(args.single_frame_t, args.single_frame_t+1)
    ...
    for t in t_rng:
      ...
      if not args.noX:
        scene.add_geometry(tmu.MakeAxes(axSize, x[t], np.tile([0, 0, 0, 255],
          [4,1]).astype(np.int), minor=axMinor))

        # draw parts here for mesh
        if not args.noTheta:
          print(theta.shape)
          K = theta.shape[1]
          for k in range(K):
            ...
            scene.add_geometry(tmu.MakeAxes(axSize, T_world_part, c, minor=axMinor))
      ...
      if args.world_axis:
        ...
        scene.add_geometry(tmu.MakeAxes(axSize, W,
          np.tile(gray, [4,1]).astype(np.int), minor=axMinor))

      if not args.orbit:
        if args.save:
          tmu.save_render(scene, fnames[t], res=[1920,1080])
        else:
          tmu.show_scene_with_bindings(scene, res=[1920,1080], point_eps=args.point_eps)
      else:
        nCams = 8
...
if __name__ == "__main__":
  ...
  parser.add_argument('resultPath', type=str, help='result file')
  ...
  parser.add_argument('--single_frame', action='store_true', help="Just show single frame")
  parser.add_argument('--single_frame_t', type=int, default=0, help="which single frame to show")
  ...
  parser.add_argument('--point_eps', type=float, default=0.25, help="point size increment/decrement")
  parser.add_argument('--world_axis', action='store_true', help="draw world axis")
